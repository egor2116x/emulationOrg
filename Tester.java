...
public class Tester extends IWorker {
	
	private Director director;
	private Accountant accountant;

	public Tester(int id, int number, int sh, Director director, Accountant accountant) {
	...
	public void run() {
		...
		while(exitThread){ //main loop
			...
			if(nowFunction != null && ((new Date(System.currentTimeMillis())).getTime() - delta) / 1000 > nowFunction.getTime()){
				...
				++countWork;
				System.out.println("tester number : " + number + " func id : " + nowFunction.getId() + " to work ended " + " time :" +
			...
			if(((new Date(System.currentTimeMillis())).getTime() - time) / 1000 >= timeMonth){
				exitThread = false;
				System.out.println("Tester number " + number + " Month ended");
