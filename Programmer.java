...
public class Programmer extends IWorker {
	...
	public Programmer(int id, int number, int sh, Director director, Accountant accountant) {
		super(id, number);
	...
	public void run() {	
		...
		while(exitThread){ //main loop
			...
			if(nowFunction != null && ((new Date(System.currentTimeMillis())).getTime() - delta) / 1000 > nowFunction.getTime()){
				...
				++countWork;
				System.out.println("prog number : " + number + " func id : " + nowFunction.getId() + " to work ended " + " time :" +
			...
			if(((new Date(System.currentTimeMillis())).getTime() - time) / 1000 >= timeMonth){
				exitThread = false;
				System.out.println("Prog number " + number + " Month ended");
